...
#### 4. 회원 관리 시스템 👤
- [x] **로그인/회원가입 기능** (이메일/비밀번호 기반)
- [x] **무료 회원**: 2회 체험 제한
- [x] **프리미엄 회원**: 무제한 분석
- [x] 사용자 프로필 관리 (나이, 성별, 키, 몸무게, 활동량, 목표)
- [x] localStorage 기반 세션 관리
- [x] **관리자 페이지** (`/admin`)
  - 아이디: `admin@calcare.ai`
  - 비밀번호: `admin1234`
  - 전체 회원 목록 조회
  - 통계 대시보드 (전체/무료/프리미엄 회원 수, 오늘 분석 횟수)
  - 회원 프리미엄 업그레이드 기능
...
#### 9. 자동응답봇 FAQ 시스템 🤖
- [x] 6개 언어 완전 번역 (한/영/중/일/베트남/아랍)
- [x] 9개 주요 카테고리 (일반문의, 사진인식, 칼로리분석, 질병예측, 계정관리, 프리미엄결제, 앱사용법, 오류해결, 보안·데이터)
- [x] 카테고리별 질문-답변 구조
- [x] 모바일 최적화 클릭식 확장/축소 UI
- [x] 아랍어 RTL 레이아웃 자동 전환
- [x] CALCARE AI 디자인 시스템 적용

#### 10. 건강정보 안내 - 공개 식품/영양 DB 목록 📊
- [x] 6개 언어 완전 번역 (한/영/중/일/베트남/아랍)
- [x] 5개 공개 DB 연동 정보
  - 🇰🇷 MFDS 식품영양성분 데이터베이스 (한국 1순위 DB)
  - 🇰🇷 농촌진흥청 국가표준식품성분표 (3,300종 식품)
  - 🇺🇸 USDA FoodData Central (글로벌 식품 DB)
  - 🇨🇦 FooDB (28,000+ 화학성분 데이터)
  - 🌍 Open Food Facts (수백만 제품 바코드 DB)
- [x] 질병 경고 시스템 작동 원리 (5단계 워크플로우)
- [x] 4가지 질병 위험도 분석 (비만, 당뇨, 고혈압, 고지혈증)
- [x] DB 연동 장점 및 활용 팁
- [x] 각 DB별 API URL, 웹사이트 링크 제공
- [x] 모바일 최적화 카드 기반 UI

#### 11. 영양성분 정보 섹션 🍃
- [x] 6개 언어 완전 번역 (한/영/중/일/베트남/아랍)
- [x] 칼로리로 직접 확인 가능한 정보 (5가지)
  - 총 열량 (kcal per serving, 100g, 실제 섭취량)
  - 1일 권장 섭취량 대비 비율
  - 다이어트/감량 목표와의 차이
  - 칼로리 소모 운동량 환산
  - 다른 음식과 칼로리 비교
- [x] 칼로리로 계산 가능한 정보 (4가지)
  - 체중 변화 예측 (7,700kcal = 1kg 지방)
  - 기초대사량(BMR) 대비 비교
  - 총 에너지 소비량(TDEE) 균형
  - 식습관 패턴 분석
- [x] 칼로리 데이터로 가능한 위험 신호 (6가지)
  - 비만 위험도
  - 지방간 위험
  - 심혈관 질환 위험
  - 당뇨 위험 (간접적)
...
- [x] 칼로리만으로는 불가능한 정보 (6가지)
  ...
  - 비타민/미네랄 결핍
  - 식이섬유/콜레스테롤 정보
- [x] 결론: 통합 시스템의 필요성 (칼로리 + 영양성분 + 개인 건강정보 + AI 음식 인식)
- [x] 모바일 최적화 UI (카드 기반 디자인, 호버 효과, 그라데이션)
...
| Method | Path | 파라미터 | 설명 |
|--------|------|---------|------|
| POST | `/api/auth/register` | email, password, name | 회원가입 |
...
| POST | `/api/user` | email, name, age, gender, height, weight, activity_level, goal | 사용자 생성/업데이트 (레거시) |
| PUT | `/api/user/:id` | name, age, gender, height, weight, activity_level, goal | 사용자 프로필 업데이트 |
| POST | `/api/user/:id/upgrade` | - | 프리미엄 업그레이드 |
...
| POST | `/api/intake` | user_id, food_name, serving_size, calories, protein, carbs, fat, sugar, sodium, meal_type | 음식 섭취 기록 (무료 회원 2회 제한) |
...
| Method | Path | 파라미터 | 설명 |
|--------|------|---------|------|
| GET | `/api/admin/stats` | - | 전체 통계 (전체/무료/프리미엄 회원, 오늘 분석) |
...
| Path | 설명 |
|------|------|
| `/` | 메인 대시보드 (음식 분석, 통계, 로그인/회원가입) |
| `/admin` | 관리자 페이지 (로그인 필요: admin@calcare.ai / admin1234) |
| `/faq.html` | 자동응답봇 FAQ (6개 언어 지원) |
| `/health-info.html` | 건강정보 안내 - 공개 식품/영양 DB 목록 (6개 언어 지원) |
| `/nutrition-info.html` | 영양성분 정보 - 칼로리 기반 분석 및 한계 (6개 언어 지원) |
